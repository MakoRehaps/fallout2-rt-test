...
def add_subway_surface_exits(outgoing,path):
    ...
    for e in doc.get('surface_exits',[]):
        ...
        # Metro entrances belong near the location center rather than map edge.
        outgoing[name].append({
            'side':'metro','x':int(e.get('x',50)),'y':int(e.get('y',50)),'rotation':0,
            'target_map':int(e['target_map_id']),'target_name':e['target_map'],
            'target_tile':int(e.get('target_tile',20100)),
            'target_elevation':int(e.get('target_elevation',0)),
            'target_rotation':int(e.get('target_rotation',0)),
        }); added+=1
    return added
...
def main():
    ...
    ap.add_argument('--subway', type=Path, help='Metro graph/job description')
    ap.add_argument('--subway-build', type=Path, help='Generated subway manifest containing surface->metro exits')
    ...
    metro_surface_exits=add_subway_surface_exits(exits,a.subway_build)
    doc=json.loads(a.structures.read_text(encoding='utf-8'))
    index={'format':'PhoBoi.F3OIndex/4','maps':[]}
    for mp in doc.get('maps',[]):
        ...
        for e in exits.get(name,[]):
            lines.append(f"EXIT {e['side']} {e['x']} {e['y']} {e['target_map']} {e['target_tile']} {e['target_elevation']} {e['target_rotation']} target={e['target_name']}")
        out=a.output/(name+'.F3O'); out.write_text('\n'.join(lines)+'\n',encoding='utf-8')
        installed=None
        if a.maps_dir:
            installed=a.maps_dir/out.name; shutil.copy2(out, installed)
        index['maps'].append({'map_name':name,'file':out.name,'installed_sidecar':str(installed) if installed else None,'commands':len(lines)-3,'exits':len(exits.get(name,[]))})

    if a.subway and a.subway.exists():
        subway=json.loads(a.subway.read_text(encoding='utf-8'))
        (a.output/'subway_runtime_jobs.json').write_text(json.dumps({'format':'PhoBoi.FO3SubwayRuntimeJobs/1','jobs':subway.get('generation_jobs',[]),'links':subway.get('links',[])},indent=2),encoding='utf-8')
    ...
    print(f"Surface runtime exits: {sum(len(v) for v in exits.values())} (metro={metro_surface_exits})")
    if a.maps_dir: print(f"Installed F3O sidecars -> {a.maps_dir}")
