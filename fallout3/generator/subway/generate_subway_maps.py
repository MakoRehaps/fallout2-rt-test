...
F3O files produced here are build-time intermediates only. They are written to
an isolated workspace and are not installed beside playable MAP files.
"""
...
def edge_exit(side,target_id,target_name):
    spots={'north':(50,2,3),'south':(50,97,0),'west':(2,50,1),'east':(97,50,4)}
    x,y,rot=spots[side]
...
def main():
    ...
    ap.add_argument('--f3o-dir',required=True,type=Path,help='Build-time F3O workspace; not installed into playable MAPS')
    ap.add_argument('--output',required=True,type=Path)
    ap.add_argument('--seed',type=int,default=3003)
    a=ap.parse_args()
    a.output.mkdir(parents=True,exist_ok=True)
    a.maps_dir.mkdir(parents=True,exist_ok=True)
    a.f3o_dir.mkdir(parents=True,exist_ok=True)
    ...
    maps=[]; name_counter=0; station_nodes={}
    ...
    def make_map(kind,station=None,category='cave'):
        ...
        maps.append(rec); return rec

    for s in graph.get('stations',[]):
        sid=s['id']; concourse=make_map('station_concourse',sid,'industrial'); platform=make_map('station_platform',sid,'industrial')
    ...
    neighbors={m['map_name']:[] for m in maps}; surface_exits=[]
    ...
    for sid,n in station_nodes.items():
        connect(n['concourse'],n['platform']); surf=surface_by_name.get(sid)
        if surf:
            surface_exits.append({'surface_map':sid,'surface_map_id':int(surf['map_id']),'target_map':n['concourse']['map_name'],'target_map_id':n['concourse']['map_id'],'x':50,'y':50,'side':'metro','target_tile':20100,'target_elevation':0,'target_rotation':0})
            neighbors[n['concourse']['map_name']].append({'map_name':sid,'map_id':int(surf['map_id'])})

    by_name={m['map_name']:m for m in maps}
    for chain_info in link_chains:
        chain=[by_name[name] for name in chain_info['maps'] if name in by_name]
        for x,y in zip(chain,chain[1:]): connect(x,y)

    for m in maps:
        write_f3o(a.f3o_dir/(m['map_name']+'.F3O'),m['map_name'],m['category'],neighbors[m['map_name']][:4])
    ...
    out={'format':'PhoBoi.FO3SubwayBuild/2','maps':maps,'stations':station_nodes,'link_chains':link_chains,'surface_exits':surface_exits,'f3o_workspace':str(a.f3o_dir),'first_map_id':min((m['map_id'] for m in maps),default=None),'next_map_id':next_id}
